...
const bool ENABLE_A_EXTENSION = false; // Toggle for Atomics
...
void riscv_step(volatile uint32_t* imem, volatile uint32_t* dmem,
    ...
    // EXPERIMENT: on-chip fixed-latency memory (Comet-style) instead of DRAM.
    // ap_memory gives a BRAM-style port (~1-2 cycle deterministic access), so
    // the pipeline body has no variable m_axi latency -- the thing that forced
    // II=169. No cache is present in this core (direct imem/dmem access).
    #pragma HLS INTERFACE ap_memory port=imem depth=RAM_SIZE
    #pragma HLS INTERFACE ap_memory port=dmem depth=RAM_SIZE
    ...
    INSTRUCTION_LOOP: while (true) {
        ...
        // With fixed-latency on-chip memory (ap_memory) and no cache-refill
        // sub-loops, the loop body is fixed-latency -- so pipelining should now
        // reach a low II (limited by the pc/regfile recurrences), unlike the
        // DRAM+cache core where the m_axi miss latency forced II=169.
        #pragma HLS PIPELINE
        csr_mcycle++;
